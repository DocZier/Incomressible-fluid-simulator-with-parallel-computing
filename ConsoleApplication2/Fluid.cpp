...
#pragma omp for
	...
	std::swap(u0.values, u.values);
	std::swap(v0.values, v.values);
...
#pragma omp for
	...
	DiffusionDensity();
	std::swap(density0.values, density.values);
	AdvectionDensity();
}
...
#pragma omp parallel for
			for (int j = 1; j <= M; j++)
			{
				p.values[i + j * (N + 2)] = (div.values[i + j * (N + 2)] + t.values[i - 1 + j * (N + 2)] + t.values[i + 1 + j * (N + 2)] + t.values[i + (j - 1) * (N + 2)] + t.values[i + (j + 1) * (N + 2)]) / 4;
			}
		}
	}
...
#pragma omp parallel for
	for (int i = 1; i <= N; i++)
	{
			f.values[0 + (N + 2) * i] = (d == 1 ? -t.values[1 + (N + 2) * i] : t.values[1 + (N + 2) * i]);
			f.values[(N + 1) + (N + 2) * i] = (d == 1 ? -t.values[N + (N + 2) * i] : t.values[N + (N + 2) * i]);
			f.values[i + (N + 2) * 0] = (d == 2 ? -t.values[i + (N + 2) * 1] : t.values[i + (N + 2) * 1]);
			f.values[i + (N + 2) * (N + 1)] = (d == 2 ? -t.values[i + (N + 2) * (N)] : t.values[i + (N + 2) * (N)]);
	}
#pragma omp parallel
	{
		f.values[0 + (N + 2) * 0] = 0.5 * (t.values[1 + (N + 2) * 0] + t.values[0 + (N + 2) * 1]);
		f.values[0 + (N + 2) * (N + 1)] = 0.5 * (t.values[1 + (N + 2) * (N + 1)] + t.values[0 + (N + 2) * (N)]);
		f.values[N + 1 + 0] = 0.5 * (t.values[N + 0] + t.values[N + 1 + 1]);
		f.values[N + 1 + (N + 2) * (N + 1)] = 0.5 * (t.values[N + (N + 2) * (N + 1)] + t.values[N + 1 + (N + 2) * (N)]);
	}
}
